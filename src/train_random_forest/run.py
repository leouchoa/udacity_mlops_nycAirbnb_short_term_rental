...
def delta_date_feature(dates):
    ...
    by pd.to_datetime), it returns the delta in days between
...
def go(args):
    ...
    logger.info(f"Downloading {args.trainval_artifact}")
    trainval_local_path = run.use_artifact(args.trainval_artifact).file()

    X = pd.read_csv(trainval_local_path)
    y = X.pop("price")  # remove "price" column from X and puts it into y

    logger.info(f"Minimum price: {y.min()}, Maximum price: {y.max()}")

    logger.info("Make train-validation split")
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # Ordinal categorical are categorical values for which
    # the order is meaningful. Example:
    ...
    # production (nor during training). That isn't true for neighbourhood_group
    #
    ...
    # - Create a feature that represents the number of days passed
    ...
    #
    # 1. step called "preprocessor" applying `preprocessor`
    # 2. step called "random_forest" with the random forest instance that
    # # we just saved in the `random_forest` variable.
    #
    sk_pipe = Pipeline(
        steps=[
